        ...
        <div class="py-12 px-6 h-full flex flex-col gap-6">
            ...
            <div class="flex items-center gap-x-6 mt-4">
                <div class="border rounded-xl h-20 w-[6.25rem] overflow-hidden" id="previewContainer">
                    <img src="{{ asset('images/complex-placeholder.jpg') }}" class="w-full h-full object-cover" alt=""
                        id="previewImage" />
                </div>
                <div class="flex flex-col gap-1">
                    <span class="text-base font-bold text-text2 tracking-wide" id="previewName">Новый комплекс</span>
            ...
            <form class="flex flex-wrap gap-6 form" id="createComplexForm"
                action="{{ route('storeComplex', auth()->user()->id) }}" method="POST" enctype="multipart/form-data"
                novalidate>
                @csrf
                ...
                <div class="form-item w-full lg:w-[calc(50%-1.5rem)]">
                    ...
                    <div class="rounded-3xl border-auth-input-border-color border px-4 h-12.5 flex items-center mt-1">
                        <select name="type" id="complexType"
                            ...
                            <option value="residential">ЖК</option>
                            <option value="hotel">ГК</option>
                        </select>
                    </div>
                </div>
                ...
                {{-- <div class="form-item w-full lg:w-[calc(50%-1.5rem)]">
                    <label for="complexSort" class="text-input-divider text-xs font-medium tracking-wide pl-2">Порядок
                        сортировки:</label>
                    <div class="rounded-3xl border-auth-input-border-color border px-4 h-12.5 flex items-center mt-1">
                        <input type="number" name="sort" id="complexSort" value="0" min="0"
                            class="h-12.5 text-input-divider text-sm font-semibold tracking-wide w-full outline-none border-none"
                            placeholder="0" />
                    </div>
                </div> --}}
                ...
                {{-- <div class="form-item w-full lg:w-[calc(50%-1.5rem)]">
                    ...
                    <div class="rounded-3xl border-auth-input-border-color border px-4 h-12.5 flex items-center mt-1">
                        <select name="popular" id="complexPopular"
                            ...
                            <option value="1">Популярный</option>
                        </select>
                    </div>
                </div> --}}
                ...
                <div class="form-item w-full">
                    ...
                    <div class="mt-1">
                        <textarea name="content" id="complexContent" data-simple-editor data-height="200px"
                            data-placeholder="Описание комплекса..."></textarea>
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            complexNameInput.addEventListener('input', function() {
                ...
                previewName.textContent = name ? `${prefix} "${name}"` : 'Новый комплекс';
            });
            ...
            // Form submission
            document.getElementById('createComplexForm').addEventListener('submit', function(e) {
